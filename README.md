...
Session compiles now reuse unchanged transformed local-module and virtual-JS-module output when the rewritten source and observed dependency context are unchanged. Cache keys are based on the module's rewritten source, JSX settings, and the virtual/package imports previously observed for that module, so unrelated virtual modules or unused package snapshot changes no longer invalidate every local transform. Package graph reuse is also keyed to active package roots instead of every supplied `packageFiles` entry, so unused package snapshots do not invalidate the active package graph while newly added candidate files inside active packages are still observed. The session still performs a conservative reachability walk and emits a complete Worker Loader module map on every successful compile because Dynamic Workers do not support partial module-map updates. Worker Loader also caches by ID, so changed code should use a new revision/hash-based ID when passed to `env.LOADER.get(id, callback)`. This cache reduces builder compile latency; it does not remove Dynamic Worker startup parsing/compilation for a newly loaded ID/code pair. The experimental API name is intentional: later work can refine dependency-aware graph traversal behind the same shape without making the current metadata stable.
...
- `npm run test:session-cache` records local workerd/Vitest timing signals comparing cold `compileDynamicWorker()` with session initial compiles, cached leaf updates, unrelated virtual module updates, entrypoint graph updates, active package updates, unused package snapshot updates, and active package candidate insertion, including transformed/reused/dropped and graph scanned/reused module counts;
