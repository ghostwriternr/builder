...
describe("diagnostics", () => {
  test("parse failures are normalized and source-aware", async () => {
    const [diagnostic] = expectFailure(
      await parse({ filename: "src/broken.tsx", source: BROKEN_TSX }),
    );
  ...
  test.each([["parse", parse] as const, ["transform", transform] as const])(
    "%s spans use JavaScript string offsets",
    async (phase, operation) => {
      const source = `const café = 1;\n${BROKEN_TSX}`;
      const [diagnostic] = expectFailure(
        await operation({ filename: `src/non-ascii-${phase}.tsx`, source }),
      );
