...
- Workers runtime tests verify:
  ...
  - Type-only imports/exports are ignored for runtime graph purposes, caller-provided virtual bare modules (`string`, `{ js }`, `{ json }`, `{ text }`, `{ data }`, or `{ wasm }`) can satisfy imports like `react/jsx-runtime`, JS virtual modules can import each other, transform-generated unknown bare imports are rejected before Worker Loader, and missing local relative imports, dynamic imports, unsupported bare imports, supported package graph failures, and unsupported virtual module content shapes return structured diagnostics. Common graph/import/package diagnostics now carry source-aware `file`, 1-based `line`, 1-based `column`, and `span: { start, end }` fields when parser or require-scanner offsets are available. Oxc transform source maps are usable through wasmkernel and are now consulted internally for post-transform diagnostics when generated positions have real original mappings. Unmapped generated code, including Oxc's injected automatic JSX runtime import line, correctly falls back to generated module diagnostics.
...
The constrained resolver experiment now implements those narrow pieces for exact in-memory snapshots: package exports resolution with condition selection (`workerd`, `worker`, `browser`, `import`, `require`, `default`), static ESM and literal CJS `require()` scanning/rewrite, package-root boundary validation, and diagnostics for unsupported package shapes. Package graph diagnostics now include source locations where the package resolver has offsets: dynamic CJS `require(name)`, unresolved literal CJS `require("./missing.cjs")`, unresolved ESM package imports, and ESM dynamic imports point at the package file/import expression that caused the failure. A prior-art pass over `cjs-module-lexer`, Rollup/plugin-commonjs, Rollup, and esbuild showed mature tools use lexer/parser semantics rather than whole-source regex matching. This spike keeps a tiny workerd-safe scanner, but now masks line comments, block comments, quoted strings, template literal text, and simple regex literals so fake `require()`/`exports`/`module.exports` text does not create package graph edges, force ESM package files down the CJS path, or produce misleading diagnostics. Template expression bodies (`${...}`) remain scannable executable code, with nested comments/strings/templates masked inside those expressions. This is still not a full CommonJS parser: there is no scope/shadowing analysis, npm fetching, full Node resolution, CSS/assets pipeline, dynamic import/require support, or Rolldown bundling.
